...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_curso)){
        $curso_err = "Please enter an email.";     
    } else{
        $curso = $input_curso;
    }
    ...
    if(empty($input_CPF)){
        $CPF_err = "Please enter the password amount.";     
    } else{
        $CPF = $input_CPF;
    }
    
    // Check input errors before inserting in database
    if(empty($name_err) && empty($curso_err) && empty($CPF_err)){
        // Prepare an update statement
        $sql = "UPDATE alunos SET nome=?, curso=?, CPF=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_curso, $param_CPF, $param_id);
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($curso_err)) ? 'has-error' : ''; ?>">
                            <label>Curso</label>
                            <textarea name="curso" class="form-control"><?php echo $curso; ?></textarea>
